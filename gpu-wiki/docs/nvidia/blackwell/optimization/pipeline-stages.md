...
## Blackwell Pipeline Choices

| Pipeline | Role | Typical use |
|---|---|---|
| `PipelineTmaUmma` | TMA load → tcgen05 MMA | GEMM and FMHA mainloops |
| `PipelineUmmaAsync` | TMEM accumulator producer/consumer | MMA–epilogue overlap and double buffering |
| `PipelineTmaStore` | Asynchronous TMA write-back | Epilogues with TMA stores |

Compared with Hopper, Blackwell places the accumulator in TMEM and can assign
independent TMA, MMA, and epilogue warp roles. Pipeline selection should follow
the producer/consumer edges rather than a fixed stage count.
