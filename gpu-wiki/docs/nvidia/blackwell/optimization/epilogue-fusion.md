...
```python
# GEMM + SwiGLU: do not materialize gate_out or up_out in global memory.
gate_out = gemm(x, w_gate)
up_out = gemm(x, w_up)
y = silu(gate_out) * up_out

# GEMM + RMSNorm + quantization: keep the output in the epilogue path.
out = gemm(x, w)
out_fp4, scale = fp4_quantize(rmsnorm(out))
```

Fusion helps in proportion to the intermediate traffic it removes. It also
raises TMEM, shared-memory, and epilogue-warp pressure, so validate the fused
kernel against a split baseline on the target shape.
